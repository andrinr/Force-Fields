...
static final float initialNoiseScale = 2;
static final float initialOutwardsVelocity = 0.3;
...
void setup()
{
  ...
  for (int i = 0; i<num; i++) {
    ...
    // Position in circle
    double x = cos(((float) i)*2*PI/num);
    double y = sin(((float) i)*2*PI/num);

    points.add(
      new P(new V(x*circleRadius + width/2,y*circleRadius + height/2),
      new V(
        initialNoiseStrength *(noise((float)x*initialNoiseScale)-0.5)+initialOutwardsVelocity*x,
        initialNoiseStrength *(noise((float)y*initialNoiseScale+100)-0.5)+initialOutwardsVelocity*y
      ),
      new C(0,122,255))
    );
  }
...
class P{
  ...
  void update(){
    ...
    this.speed.x += 0.003 * ( cos(this.pos.y()/rs[1]) + sin(this.pos.x()/rs[3]+rs[5]) + sin(this.pos.y()/rs[7] + rs[0]) );
